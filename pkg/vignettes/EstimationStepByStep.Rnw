...
\begin{align}
  L(\theta) &= \prod_R \prod_K P(R, K)^{M_{RK}}\\
            &= \prod_R \prod_K \big(P(R \mid K) \cdot P(K)\big)^{M_{RK}}\\
            &= \prod_R \prod_K \big(
...
\end{equation}
Setting (\ref{eq:dlambda}) to zero and solving for $\lambda$ gives
\begin{equation}
  \hat{\lambda} = -\sum_R \sum_K M_{RK} = -N,
\end{equation}
which, when substituted into (\ref{eq:pihat-part}) again, leads to
\begin{equation}
...
expectation in (\ref{eq:betahat}), (\ref{eq:etahat}), and (\ref{eq:pihat}).
...
| R)$ is estimated by $i_{RK}/i_{R+}$, where $i_{R+} = \sum_K i(R, K)$.
Consequently,
\begin{equation}
...
\begin{align}
  L(\theta) &= \prod_R \prod_K \big(P(R \mid K) \cdot P(K)\big)^{M_{RK}}\\
            &= \prod_R \prod_K P(R \mid K)^{M_{RK}}
              \cdot \big(\prod_{q \in K} g_q \!
...
\begin{align}
  \ell(\theta)
    ...
    &+ \sum_R \sum_K M_{RK} \big(
      \!\sum_{q \in K}              \!\log g_q +
      \!\sum_{q \in K^\mathcal{O}} \!\log(1 - g_{q})\big).
\end{align}
...
F.W.\ would like to thank Alice Maurer for discussing the derivation of the
estimators.
